 ...
 */
export async function seedJobInDb(prisma: PrismaClient, opts: SeedJobOpts): Promise<Job> {
  const id = randomUUID();
  return prisma.job.create({
    data: {
      ...
      inputStorageKey: opts.inputStorageKey ?? `uploads/${opts.userId}/${id}/source.bin`,
      reservedBytes: 0n,
      ...(opts.expiresAt !== undefined ? { expiresAt: opts.expiresAt } : {}),
      ...(opts.uploadExpiresAt !== undefined ? { uploadExpiresAt: opts.uploadExpiresAt } : {}),
      ...(opts.finishedAt !== undefined ? { finishedAt: opts.finishedAt } : {}),
      ...(opts.outputStorageKey !== undefined ? { outputStorageKey: opts.outputStorageKey } : {}),
