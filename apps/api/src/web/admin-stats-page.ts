...
const InnerStatsResponse = z.object({
  ...
  storage: z.object({
    usedBytes: z.string(),
    diskFree: z.object({ available: z.string(), total: z.string() }).nullable(),
  }),
  queue: z.object({
...
export const adminStatsPagePlugin: FastifyPluginAsync = async (app) => {
  app.get('/admin/stats', { preHandler: app.requireAdminSession }, async (req, reply) => {
    ...
    const jobRows = Object.entries(data.jobs)
      .map(([status, count]) => ({
        status,
        count,
        labelKey: KNOWN_JOB_STATUSES.has(status) ? `stats_jobs_${status}` : 'stats_jobs_unknown',
      }))
